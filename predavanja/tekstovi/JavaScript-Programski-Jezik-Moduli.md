...
**Пример.** Илуструје модуларно програмирање преко контруктора. И овде је задатак исти као у претходном примеру: потребно је извршити прорачунавање (квадрирање) над датим аргументом. При томе, вредност аргумента је енкапсулирана у модулу, исто као и функција која врши прорачунавање.
...
Као и у претходном примеру, да би било обезбеђено учитавање модула, користиће се спољашњи АPI веб прегледача. И ovde iзвршавање почиње од датотеке `index.html`. И у овом примеру је редослед учитавања модула експлицитно дат редоследом `<script>` елемената: прво учитава `vrednost.js`, па потом `proracun.js`.
...
Исто као у претходном примеру, евентуална промена редоследа учитавања тј. промена редоследа `<script>` елемената би довела до грешке у извршавању. &#9608;
...
Уз ЕС6 стиже подршка за модуларни систем уграђена и у сам језик – ECMAScript 6 модули. Тиме је  интегрисана подршка за модуларно програмирање:
...
Нова синтакса још увек није подржана од стране свих прегледача, па је потребно користи алате (нпр. Babel) да би се транспилирала у ЕС5. Тај сценарио је прихватљив и за извршавање у оквиру прегледача и на серверској страни, само што захтева инсталацију додатних библиотека.
