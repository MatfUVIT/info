...
2. Парсирање - узима се ток (низ) токена и од њих се гради тзв. дрво апстрактне синтаксе (енг. AST - abstract syntax tree): дрво (стабло) елемената, који представљају граматичку структуру програма.

**Пример.** Размотритимо програм из  претходном примеру.

Дрво апсатактне синтаксе за програм

```js
   var a = 2;
```
<!--
може почети са кореном - чвором највишег нивоа који се назива VariableDeclaration, са потомком који се зове Identifier (чија је вредност а), и друго дете под називом АссигнментЕкпрес
сион, која сама има дете под називом НумерицЛитерал (чија вредност
је 2).
Генерација кода
Процес узимања АСТ-а и претварања у извршни код.
Овај део се веома разликује у зависности од језика, платформе
то је циљање и тако даље.
Дакле, уместо да се заглибимо у детаље, само ћемо махнути руком и рећи
да постоји начин да се наш претходно описани АСТ узме за вар а
= 2; и претворите га у скуп машинских инструкција за стварно креирање

променљива која се зове а (укључујући резервисање меморије, итд.), а затим
сачувати вредност у а.
Детаљи о томе како мотор управља системским ресурсима
су дубље него што ћемо копати, па ћемо то узети здраво за готово
да је мотор у стању да креира и складишти променљиве као
потребна.
ЈаваСцрипт механизам је много сложенији од само ова три
корака, као и већина других компајлера језика. На пример, у
процес рашчлањивања и генерисања кода, свакако постоје кораци за
оптимизовати перформансе извршења, укључујући и сувишно смањење
елементи итд.
Дакле, овде сликам само широким потезима. Али мислим да ћете видети
укратко зашто су ови детаљи које покривамо, чак и на високом нивоу, релевантни.
Као прво, ЈаваСцрипт мотори не добијају луксуз (као други језици
компајлери) да имају довољно времена за оптимизацију, јер Јава-
Компилација скрипте се не дешава у кораку изградње пре времена, као
са другим језицима.
За ЈаваСцрипт, компилација која се дешава се дешава, у многим случајевима,
само микросекунде (или мање!) пре него што се код изврши. Да обезбеди
најбрже перформансе, ЈС мотори користе све врсте трикова (као што су ЈИТ,
које лењо компајлирају па чак и вруће поново компајлирају, итд.) које су далеко изнад
„опсег“ наше дискусије овде.
Рецимо само, ради једноставности, да сваки исечак ЈаваСцрипт-а мора
бити састављен пре (обично непосредно пре!) извршења. Дакле, ЈС компајлер
ће узети програм вар а = 2; и прво га компајлирајте, а затим
будите спремни да га извршите, обично одмах.
-->
